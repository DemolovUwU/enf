    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            if request.headers.get('HX_Request'):
                return redirect('cart:cart-modal')
            else:
                return JsonResponse({
                    'success': True,
...
class RemoveCartItemView(CartMixin, View):
    def post(self, request, item_id):
        cart = self.get_cart(request) #request to init id of user cart

        try:
            cart_item = cart.items.get(id=item_id)
            cart_item.delete()

            request.session['cart_id'] = cart.id #request.session means coockie of user
            request.session.modified = True

            context = {
                'cart': cart,
                'cart_idtems': cart.items.select_related(
                    'product',
                    'product_size__size'
                ).order_by('-added_at')
            }
